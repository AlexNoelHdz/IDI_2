...
class Perceptron:
    def __init__(self, data_location, numero_entradas, numero_salidas):
        ...
        w_h, w_o, E, epocas = self.train_data(N, M, x, d, w_h, w_o)

        y_res = self.test_data(df, N, w_h, w_o)

        self.print_results(E,epocas, y_res, df[:,N:])
    ...
    def train_data(self, N, M, x, d, w_h, w_o):
        E = inf
        alpha = 1
        epocas = 0
    ...
    def print_results(self, error, epocas, y_res, salidas_originales):
